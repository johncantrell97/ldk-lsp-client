	...
	NS::Target: NodeSigner,
{
	...
	pub(crate) fn htlc_intercepted(
		...
		if let Some(channel) = channels_by_scid.get_mut(&scid) {
			if let Some(fees) = &channel.fees {
				// this is wrong, channel amount is determined by us -- fee calculation is for when we need to forward
				let fee = {
					let proportional_fee =
						(inbound_amount_msat / 1_000_000) * fees.proportional as u64;
					if proportional_fee > fees.min_fee_msat {
						proportional_fee
					} else {
						fees.min_fee_msat
					}
				...
				let amt_to_forward_msat = inbound_amount_msat - fee;
				channel.amt_to_forward_msat = Some(amt_to_forward_msat);
				channel.intercept_id = Some(intercept_id);

				let channel_value_satoshis = (amt_to_forward_msat * 20) / 1000;

				if channel_value_satoshis > 0 {
					...
					self.enqueue_event(Event::LSPS2(crate::JITChannelEvent::FailInterceptedHTLC {
						intercept_id,
					}));
				}
			} else {
	...
	pub(crate) fn channel_ready(
		...
		println!("channel_ready received");

		let channels_by_scid = self.channels_by_scid.read().unwrap();

		if let Ok(scid) = user_channel_id.try_into() {
			println!("user_channel_id might be scid: {:?}", scid);

			if let Some(channel) = channels_by_scid.get(&scid) {
				println!("found channel to forward payment over");
				self.enqueue_event(Event::LSPS2(crate::JITChannelEvent::ForwardInterceptedHTLC {
					intercept_id: channel.intercept_id.unwrap(),
					next_hop_channel_id: *channel_id,
					next_node_id: *counterparty_node_id,
					amt_to_forward_msat: channel.amt_to_forward_msat.unwrap(),
				}));
			} else {
				println!("could not find a channel with that scid");
			}
		} else {
			println!("could not parse user_channel_id into u64 scid");
		}
	}
