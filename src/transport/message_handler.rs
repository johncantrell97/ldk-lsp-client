...
pub struct LiquidityManager<
	ES: Deref + Send + Sync + 'static + Clone,
...
impl<
		...
		OM: Deref + Send + Sync + 'static,
		NS: Deref + Send + Sync + 'static,
	> LiquidityManager<ES, Descriptor, L, RM, CM, OM, NS>
where
	ES::Target: EntropySource,
	L::Target: Logger,
	RM::Target: RoutingMessageHandler,
	...
	NS::Target: NodeSigner,
{
	...
	///
	/// Will generate a [`crate::JITChannelEvent::FailInterceptedHTLC`] event if the scid matches a payment we are expecting
	...
	///
	/// Will generate a [`crate::JITChannelEvent::ForwardInterceptedHTLC`] event if it matches a channel
	...
	pub fn channel_ready(
		...
		if let Some(lsps2_message_handler) = &self.lsps2_message_handler {
			lsps2_message_handler.channel_ready(user_channel_id, channel_id, counterparty_node_id);
		}
	}
...
impl<
		...
		OM: Deref + Send + Sync + 'static,
		NS: Deref + Send + Sync + 'static,
	> CustomMessageReader for LiquidityManager<ES, Descriptor, L, RM, CM, OM, NS>
where
	ES::Target: EntropySource,
	L::Target: Logger,
	RM::Target: RoutingMessageHandler,
...
impl<
		...
		OM: Deref + Send + Sync + 'static,
		NS: Deref + Send + Sync + 'static,
	> CustomMessageHandler for LiquidityManager<ES, Descriptor, L, RM, CM, OM, NS>
where
	ES::Target: EntropySource,
	L::Target: Logger,
	RM::Target: RoutingMessageHandler,
